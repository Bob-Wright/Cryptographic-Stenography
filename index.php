...
function steg_hide($maskfile, $hidefile)
{
	...
	} while(strpos($data,$boundary)!==false && strpos('Secret.txt',$boundary)!==false);

	// add boundary to data
	$data = $boundary.'Secret.txt'.$boundary.$data.$boundary;
	...
	if(strlen($data)*8 > ($attributes[0]*$attributes[1])*3)
	{
		...
		ImageDestroy($outpic);
		ImageDestroy($pic);
		return "Cannot fit ".'Secret.txt'." in ".$maskfile['name'].".<br />"."Secret.txt"." requires mask to contain at least ".(intval((strlen($data)*8)/3)+1)." pixels.<br />Maximum filesize that ".$maskfile['name']." can hide is ".intval((($attributes[0]*$attributes[1])*3)/8)." bytes";
	}
	...
	$name_image=rand(0,100)."encoded.png";
...
function steg_recover($image)
{
	...
	$attributes = getImageSize($image['tmp_name']);
	$pic = ImageCreateFromPNG($image['tmp_name']);
...
<head>
  ...
  <meta http-equiv="X-UA-Compatible" content="IE=edge">
	<meta property='og:title' content='Stego App' />			
	<meta property='og:site_name'  content='from Kodein' />
	<meta property='og:image' content='http://security.cs.umass.edu/cyber-biglock.jpg' />
	<meta property='og:description' content='STEGANOGRAPHY APPLICATION USING LEAST SIGNIFICANT BIT (LSB) METHOD</BR>COMBINED WITH RC4 ENCRYPTION AND BASE 64 ENCODE' />
...
<div class="col-10 col-offset-1" style="background:#d0d0ff; margin-bottom:20px;">
<h3 align=center>STEGANOGRAPHY APPLICATION USING LEAST SIGNIFICANT BIT (LSB) METHOD</BR>COMBINED WITH RC4 ENCRYPTION AND BASE 64 ENCODE</h3>
</div>
<div class="col-10 col-offset-1" style="background:#d0d0d0; margin-bottom:50px;">
  
  <!-- /.login-logo -->
  <div class="login-box-body">
	<p align=center  style="margin-bottom:20px"> <i class="fa fa-shield"></i> &nbsp from Kodein Data Security </p>
	<div class="nav-tabs-custom">
		<?php
if(!empty($_FILES['image']['tmp_name'])) {
	$result = steg_recover($_FILES['image']);
	// decode base 64
	...
	echo "
		<table border=0 class='table table-bordered' style='font-size:large'>
			<tr>
				<td align=right><b>Ciphertext Base64:</b></td>
			...
			<tr>
				<td align=right><b>Ciphertext RC4:</b></td>
...
?>
        <ul class="nav nav-tabs pull-right">
            <li ><a href="#tab_1" class="btn btn-app" data-toggle="tab" ><i class="fa fa-unlock"></i> Decrypt</a> </li>
            <li class="active"><a href="#tab_2" class="btn btn-app" data-toggle="tab" ><i class="fa fa-lock"></i> Encrypt</a></li>
        </ul>
            <div class="tab-content">
              <div class="tab-pane" id="tab_1">
                ...
                <form action="<?php $_SERVER['PHP_SELF']?>" method="post" enctype="multipart/form-data">
				  <div class="form-group has-feedback">
					<input type="text" name="decrypt_key" id="decrypt_key" class="form-control" placeholder="Enter Secret Key" required>
					<span class="fa fa-key form-control-feedback"></span> 
				  </div>
				  <label>Message Cover Image (jpeg): </label>
              ...
              <div class="tab-pane active" id="tab_2">  
				...
				<form id="form_stegano" action="<?php $_SERVER['PHP_SELF']?>" method="post" enctype="multipart/form-data">
				  <div class="form-group has-feedback">
					<input type="text" id="encrypt_key" name="key" class="form-control" placeholder="Enter Secret Key">
					<span class="fa fa-key form-control-feedback"></span>
				  </div>
				  <div class="form-group has-feedback">
					<textarea id="secret" name="secret" class="form-control" rows=3 placeholder="Enter Secret Message" required></textarea>
					<span class="fa fa-file-text-o form-control-feedback"></span>
				  </div>
				  <label>Message Cover Image (jpg): </label>
				  <div class="input-group" style="margin-bottom:30px">
					<span class="input-group-addon"><i class="fa fa-image"></i></span>
...
<!--	<div class="col-xs-6 col-md-4" style="margin-bottom:20px;" align=center>
		<a href="https://wikipedia.org/wiki/Steganography" target="_blank" class="btn btn-success btn-block btn-flat">Steganography</a> 
	</div>
	
	<div class="col-xs-6  col-md-4" style="margin-bottom:20px;" align=center>
		<a href="https://wikipedia.org/wiki/Cryptography" target="_blank" class="btn btn-success btn-block btn-flat">Cryptography</a>
	</div>
	
	<div class="col-xs-6  col-md-4" style="margin-bottom:20px;" align=center> 
		<a href="https://en.wikipedia.org/wiki/RC4" target="_blank" class="btn btn-success btn-flat btn-block">RC4 Encryption</a>
	</div>
	
	<div class="col-xs-6 col-md-4" style="margin-bottom:20px;" align=center> 
		<a href="https://en.wikipedia.org/wiki/Base64" target="_blank" class="btn btn-success btn-flat btn-block">BASE 64 Encryption</a>
	</div>
	
	<div class="col-xs-12" style="margin-bottom:20px;">
		<p align=justify> This application is not connected to a database, all images and confidential information entered are not stored on the server. </p>
